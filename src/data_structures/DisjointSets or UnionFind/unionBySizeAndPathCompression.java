...
public class WeightedQuickUnion {
    ...
    private int find(int i) {
        ...
        // path compression:-
        // this is better than above as this connects every node to root
        // previous path comp connected every node to its grandparent not root
        while(i != root) {
            int next = id[i];
            id[i] = root;
            i = next;
    ...
    public void union(int p, int q) {
        ...
        if(pRoot == qRoot) return; // elements are in same group 
